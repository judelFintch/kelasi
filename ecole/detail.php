...
<div  role="alert">
     <span class="list-group-item active">Parcours Scolaire</span>
		<?php		
				$i=0;
				$selection_distinc=$bdd->query("SELECT DISTINCT anne_acad FROM finance WHERE matricule='$matricule'");
...
<?php echo '
			<input type="text"  class="form-control" id="nom_md" '.$ecriture.' value='.$resultat['nom'].' <br/>
					...
					if($_SESSION['level']==6){
				echo '
					<label>categorie Eleve</label>
					<select class="form-control categorieEleve">
					<option>'.$resultat['categorie'].'</option>
