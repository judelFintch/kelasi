...
function insertionFromFinance($noms,$matricule,$classe,$mois,$devise,$usd,$cdf,$numfact,$nature,$pid,$compte,$annacad_old){
	global $bdd;
	global $taux;
	global $annacad;
	...
	if($devise=='CDF'){
	$bdd->exec("INSERT INTO finance values('','$noms','$matricule','$classe','$mois','00','$cdf','$annacad',now(),'CDF','$nature','$taux','$numfact','$pid','$compte','$annacad_old')")or die(print_r($bdd->errorInfo()));
  }
   if($devise=='USD'){

	$bdd->exec("INSERT INTO finance values('','$noms','$matricule','$classe','$mois','$usd','00','$annacad',now(),'CDF','$nature','$taux','$numfact',$pid,'$compte','$annacad_old')")or die(print_r($bdd->errorInfo()));
  }
...
////gestion des affichages des factures de la tables tampons a la facture 
    ...
    $item .= "<tr>";
			...
			$item .= "<td>".$key['libelle']."</td><td>".$key['qte']."</td><td>".$key['prix_usd']."</td><td>".$key['prix_cdf']."</td>";
...
$netcdf=round($totalcdf+$conversioncdf,1);
	$item.="tr>";
	$item .= "<td></td><td></td><b>".$netusd." $</b><td> ".$totalusd." $</td><td>T. CDF  ".$totalcdf."</td>";
	$item.="</tr>";
	
	$item.="tr>";
	$item .= "<td></td><td></td><td>Net CDF <b>".$netcdf."</b></td>";
	$item.="</tr>";
...
if(isset($_POST['mois']) AND isset($_POST['matricule']) AND isset($_POST['ChoixDevise'])){
 ...
 insertionFromFinance($noms,$_POST['matricule'],$classe,$mois,$devise,$usd,$cdf,$numfact,$nature,$pid,$compte,$annacad_old);
 	 }
    ...
    $bdd->exec("DELETE FROM facture ORDER BY id ASC LIMIT 1");
 }
