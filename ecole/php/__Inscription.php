...
//function information de la classe eleve
 ...
 //function categorie classe
  ...
  function insertionInfomationFinance($nomeleve,$matricule,$classe,$reduction,$link){
   //recuperation information promotion
     ...
     global $annacad;
     global $taux;
     $bdd->query("INSERT INTO finance values('','$nomeleve','$matricule','$classe','Frais Inscription','$frais_insert','000','$annacad',now(),'USD','$link','$taux','$codfcat','1','$compte','$annacad',1)")or die(print_r($bdd->errorInfo()));
}
 //insertions des informations dans la table inscriptions
  function InsertionsDansLatableInscription($matricule,$nomeleve,$classe,$date,$reduction,$link){
     ...
     $bdd->query("INSERT INTO frais_insciption VALUES('','$matricule','$nomeleve','$frais_insert','$classe','$annacad','$date','$codfcat')")or die(print_r($bdd->errorInfo()));
  }
  ...
  $bdd->query("INSERT INTO eleve VALUES('', '$matricule','$nom','$postnom','$prenom','$daten','$lieun','$pays','$adresse','$classe','$codep','$matricule','$annacad','$user','$date','$categorie_paiement','$categorie_cl','disponible','$sexe','$mention','$pourcentage','$provenance','$modePaiement')")or die(print_r($bdd->errorInfo()));
  }
  //fonction permetant d archives les informations des eleves pour la reinscriptions seulement
  function archivageInformationEleve($matricule){
     global $bdd;
     global $annacad;
     global $taux;
     $infoEleve=informationEleveEnreinscription($matricule);
     $matricule=$infoEleve['matricule'];
     $promotion_ancienne=$infoEleve['promotion'];
     $categorie_classe_ancienne=$infoEleve['categorie_classe'];
     $categorie_eleve_ancienne=$infoEleve['categorie'];
     $codep=$infoEleve['codep'];
     $Date_insciption_ancien=$infoEleve['date_insc'];
     $mention_acien=$infoEleve['mention'];
     $ancien_pourentage=$infoEleve['pourcentage'];
     $ancien_photo=$infoEleve['photo'];
     $annacad_acien=$infoEleve['annacad'];
     $bdd->query("INSERT INTO archives VALUES('','$matricule','$promotion_ancienne','$categorie_classe_ancienne','$codep','$annacad_acien','$Date_insciption_ancien','$categorie_eleve_ancienne','$mention_acien','$ancien_pourentage','$ancien_photo')")or die(print_r($bdd->errorInfo()));
  }
